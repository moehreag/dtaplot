...
public class WelcomeComponent extends ViewComponent {
	...
	public void draw(float width, float height) {
		...
		itemWidth = ImGui.calcTextSize(ViewComponent.tr("text.welcome.title")).x;
		ImGui.pushFont(App.getInstance().titleFont);
		ImGui.setCursorPos(width / 2 - itemWidth / 2, height / 4);
		ImGui.text(ViewComponent.tr("text.welcome.title"));
		ImGui.popFont();
		ImGui.setCursorPosY(ImGui.getCursorPosY() + 10);
		itemWidth = ImGui.calcTextSize(ViewComponent.tr("text.welcome.text")).x;
		ImGui.setCursorPosX(width / 2 - itemWidth / 2);
		ImGui.text(ViewComponent.tr("text.welcome.text"));


		itemWidth = ImGui.getStyle().getItemSpacingX() * 3;
		itemWidth += ImGui.calcTextSize(ViewComponent.tr("action.open")).x;
		itemWidth += ImGui.calcTextSize(ViewComponent.tr("action.load")).x;
		itemWidth += ImGui.calcTextSize(ViewComponent.tr("action.connect.ws")).x;
		itemWidth += ImGui.calcTextSize(ViewComponent.tr("action.connect.tcp")).x;

		ImGui.setCursorPos(width / 2 - (itemWidth / 2), height / 2 - 20);
		Dialogs.showOpenDialog("welcome.open", () -> ImGui.button(ViewComponent.tr("action.open")), FileFilters.OPEN).ifPresent(FileHandler::open);
		ImGui.sameLine();
		Dialogs.showConnectDialog("w.load", () -> ImGui.button(ViewComponent.tr("action.load"))).ifPresent(address -> {
		...
		ImGui.sameLine();
		Dialogs.showConnectDialog("w.connect.ws", () -> ImGui.button(ViewComponent.tr("action.connect.ws"))).ifPresent(address -> {
			App.getInstance().setView(App.View.WS);
			SocketLoader.loadWS(address, App.View.WS.getComponent());
		});
		ImGui.sameLine();
		Dialogs.showConnectDialog("w.connect.tcp", () -> ImGui.button(ViewComponent.tr("action.connect.tcp"))).ifPresent(address -> {
